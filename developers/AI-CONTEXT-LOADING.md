...
| 任务类型 | 最小读取集 | 可选补充 |
|---|---|---|
| 代码修改 | `AGENTS.md` + `developers/CODE-STYLE.md` + `developers/CODE-STYLES/` 对应语言/场景规范 | — |
| 文档修改 | `AGENTS.md` + `developers/DOC-RULES.md` | `developers/INDEX.md` |
| 历史追溯 | `AGENTS.md` + `developers/ANALYSIS/README.md` | `developers/REPORTS/` 文件 |
| 快照/无上下文接入 | `AGENTS.md` + `developers/INDEX.md` | CodeGraph 查询 |
...
代码索引（CodeGraph）**仅在满足以下条件之一时查询**，其余情况不主动加载：
...
| 优先级 | 工具 / 资源 | 可用性 | 回退策略 |
|---|---|---|---|
| ① | `developers/INDEX.md` | 总是可用（文件存在时） | 文件不存在则跳过 |
