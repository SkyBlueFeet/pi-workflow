...
> 本文记录当前已经完成的独立 agent 基础设施，并保留阶段 10 早期目标与实现脉络供追溯。
> 阶段 10 当前的主交付目标，以 [PLAN_PHASE-10_PI-AGENT-ASSEMBLY-IMPLEMENTATION.md](./PLAN_PHASE-10_PI-AGENT-ASSEMBLY-IMPLEMENTATION.md) 为准；该目标要求以 `pi-tui` 作为默认运行面完成装配 DSL 收口，并将外部 agent 地址引用纳入预定功能。

## 1. 最终实现目标

本阶段用于为 `pi-workflow` 重塑一套以 `PI` 为能力本体的自定义智能体系统。本文保留的是“独立智能体”基础设施背景，不再作为阶段 10 当前验收主口径；当前实施收口以专项计划中的统一装配模型、`pi-tui` 默认运行面和外部 agent 地址引用为准。
